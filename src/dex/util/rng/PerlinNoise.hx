...
class PerlinNoise
{
    ...
    /**
     * Initialize a perlin noise generator with a given seed.
     *
     * Only seeds up to `65535` are supported, higher values are wrapped around with `%`.
     */
     ...
     */
    public function noise2d(x: Float, y: Float, octaves: Int = 1, frequency: Float = 1, lacunarity: Float = 2): Float
    {
        var sum: Float = 0;
        ...
        for (i in 0...octaves)
        {
            sum += amplitude * n2d(x * frequency, y * frequency);
    ...
    public function noise1d(x: Float, octaves: Int = 1, frequency: Float = 1, lacunarity: Float = 2): Float
    {
        return noise2d(x, x, octaves, frequency, lacunarity);
    }
}
